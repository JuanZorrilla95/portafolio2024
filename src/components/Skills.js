...
const toSlug = (name) => name.toLowerCase().replace(/[^a-z]/g, "");
    ...
    <>
      {categories.map(({ icon, color, label, skills }) => (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-1.5 md:gap-2">
            {skills.map((name) => (
              <SkillIcon key={name} name={name} src={`/${toSlug(name)}.png`} />
            ))}
          </div>
        </motion.div>
      ))}
      ...
      >
        ...
        <div className="space-y-2">
          <a href="https://imgur.com/5U3XP8M" target="_blank" rel="noopener noreferrer"
          ...
          <div className="flex items-start gap-1.5 md:gap-2 text-xs md:text-sm text-gray-600 dark:text-gray-400">
            ...
            <span>
              ...
              <a href="https://www.efset.org/cert/EUPSMW" target="_blank" rel="noopener noreferrer"
